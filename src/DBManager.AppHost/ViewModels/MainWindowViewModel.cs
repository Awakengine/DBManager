...
namespace DBManager.App.ViewModels
{
    public class MainWindowViewModel : ViewModelBase
    {
        ...
        public MainWindowViewModel()
        {
            ...
            OpenTableCommand = new RelayCommand<DatabaseObjectViewModel>(OpenTable);
            DesignTableCommand = new RelayCommand<DatabaseObjectViewModel>(DesignTable);
            ExportDataCommand = new RelayCommand<DatabaseObjectViewModel>(ExportData);
            ImportDataCommand = new RelayCommand<DatabaseObjectViewModel>(ImportData);

            LightThemeCommand = ReactiveCommand.Create(() =>
            {
                MessageBus.Current.SendMessage(new ThemeChangedMessage("Light"));
            });

            DarkThemeCommand = ReactiveCommand.Create(() =>
            {
                MessageBus.Current.SendMessage(new ThemeChangedMessage("Dark"));
            });
            // 初始化示例数据
        ...
        {
            StatusMessage = "正在刷新连接列表...";
            // 实际应用中应该刷新连接列表
            StatusMessage = "连接列表已刷新";
        }

        private void RefreshDatabase()
        {
            StatusMessage = "正在刷新数据库对象...";
            // 实际应用中应该刷新数据库对象
            StatusMessage = "数据库对象已刷新";
        }

        private void OpenTable(DatabaseObjectViewModel? tableObject)
        {
            if (tableObject == null) return;

            StatusMessage = $"正在打开表 {tableObject.Name}...";

            // 创建表数据视图标签页
            var tableTab = new TabItemViewModel
            {
                Header = tableObject.Name,
                Content = new TableDataViewModel { TableName = tableObject.Name },
                CanClose = true
            };
            ...
            StatusMessage = $"已打开表 {tableObject.Name}";
        }

        private void DesignTable(DatabaseObjectViewModel? tableObject)
        {
            if (tableObject == null) return;

            StatusMessage = $"正在打开表设计器 {tableObject.Name}...";

            // 实际应用中应该打开表设计器

            StatusMessage = $"已打开表设计器 {tableObject.Name}";
        }

        private void ExportData(DatabaseObjectViewModel? tableObject)
        {
            if (tableObject == null) return;

            StatusMessage = $"正在导出表 {tableObject.Name} 的数据...";

            // 实际应用中应该打开导出对话框

            StatusMessage = $"已导出表 {tableObject.Name} 的数据";
        }

        private void ImportData(DatabaseObjectViewModel? tableObject)
        {
            if (tableObject == null) return;

            StatusMessage = $"正在导入表 {tableObject.Name} 的数据...";

            // 实际应用中应该打开导入对话框

            StatusMessage = $"已导入表 {tableObject.Name} 的数据";
        }
